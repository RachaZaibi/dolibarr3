...
/**
 *	\file       htdocs/custom/ultimatepdf/core/modules/commande/doc/pdf_ultimate_order.modules.php
 *	\ingroup    commande
...
/**
 *	Class to generate PDF orders with template ultimate_order template
 */
class pdf_ultimate_order extends ModelePDFCommandes
{
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->commande->multidir_output[$conf->entity]) {
			...
			if (file_exists($dir)) {
				...
				//Set $hautcadre
				if (!empty($conf->global->ULTIMATE_PDF_ORDER_ADDALSOTARGETDETAILS &&($conf->global->ULTIMATE_PDF_ORDER_ADDALSOTARGETDETAILS == 1)) || (!empty($conf->global->MAIN_INFO_SOCIETE_NOTE) && !empty($this->emetteur->note_private)) || (!empty($conf->global->MAIN_PUBLIC_NOTE_IN_ADDRESS) && !empty($object->note_public)) || (!empty($conf->global->MAIN_INFO_TVAINTRA) && !empty($this->emetteur->tva_intra))) {
					$hautcadre = 48;
				} else {
					$hautcadre = 40;
				}
				...
				$tab_top_newpage = (empty($conf->global->ULTIMATE_ORDERS_PDF_DONOTREPEAT_HEAD) ? $this->marge_haute + $logo_height + 10 : 18);
				...
				if ($notetoshow && empty($conf->global->MAIN_PUBLIC_NOTE_IN_ADDRESS)) {
					...
					$thisstr = '<img';
					$strpos = strpos($inthatstr, $thisstr);
					$thatstr = '/>';
					$imgsignature = between($thisstr, $thatstr, $inthatstr);				
					$slug_length = strlen($imgsignature);
					$notetoshow = substr($inthatstr, $strpos, $slug_length);*///test
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					if ($this->getColumnStatus('desc')) {
						...
						if (!empty($product->barcode) && !empty($product->barcode_type_code) && $object->lines[$i]->product_type != 9 && $conf->global->ULTIMATEPDF_GENERATE_ORDERS_WITH_PRODUCTS_BARCODE == 1 && !$conf->milestone->enabled) {
					...
					if ($this->atleastonediscount && !empty($conf->global->ULTIMATE_GENERATE_ORDERS_WITH_PUAFTER) && empty($conf->global->MAIN_GENERATE_DOCUMENTS_HIDE_DETAILS)) {
					...
					// Weight
					if ($conf->global->ULTIMATE_GENERATE_ORDERS_WITH_WEIGHT_COLUMN) {
					...
					if (!empty($conf->global->ULTIMATEPDF_GENERATE_ORDERS_WITH_PRODUCTS_BARCODE == 1 || $conf->global->ULTIMATE_PRODUCT_ENABLE_CUSTOMCOUNTRYCODE)) {
				...
				if (!empty($conf->mbisignature->enabled) && $conf->global->SIGNATURE_ENABLE_ORDER == "true") {
	...
	function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		if (!empty($conf->global->ULTIMATE_GENERATE_ORDERS_WITH_WEIGHT_COLUMN) && !empty($conf->global->ULTIMATE_GENERATE_ORDERS_WITH_TOTAL_WEIGHT)) {
			//Total Weigth
			$tmparray = $object->getTotalWeightVolume();
			$totalweight = $tmparray['weight'];
			$totalweight = showDimensionInBestUnit($totalweight, 0, "weight", $langs, isset($conf->global->MAIN_WEIGHT_DEFAULT_ROUND) ? $conf->global->MAIN_WEIGHT_DEFAULT_ROUND : -1, isset($conf->global->MAIN_WEIGHT_DEFAULT_UNIT) ? $conf->global->MAIN_WEIGHT_DEFAULT_UNIT : 'no', 0);
			//var_dump($tmparray);exit;
			//$totalweight = $object->getTotalWeightVolume();
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $titlekey = "PdfOrderTitle")
	{
		...
		if (!empty($conf->global->ULTIMATEPDF_GENERATE_ORDERS_WITH_TOP_QRCODE) || !empty($conf->global->ULTIMATEPDF_GENERATE_ORDERS_WITH_MYCOMP_QRCODE)) {
		...
		if ($showaddress) {
			...
			if (!empty($conf->global->ULTIMATE_PDF_ORDER_ADDALSOTARGETDETAILS) || (!empty($conf->global->MAIN_INFO_SOCIETE_NOTE) && !empty($this->emetteur->note_private)) || (!empty($conf->global->MAIN_INFO_TVAINTRA) && !empty($this->emetteur->tva_intra))) {
				$hautcadre = 48;
			} else {
				$hautcadre = 40;
			}
	...
	public function defineColumnField($object, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if (!empty($conf->global->ULTIMATE_ORDERS_WITH_LINE_NUMBER) && ($conf->global->ULTIMATE_PDF_BORDER_LEFT_STATUS == 'yes') || !empty($conf->global->ULTIMATE_DOCUMENTS_WITH_REF) && $this->atleastoneref == true && $conf->global->ULTIMATE_PDF_BORDER_LEFT_STATUS == 'yes') {
	...
	protected function _signature_area_simple_or_double(&$pdf, $object, $posy, $outputlangs)
	{
		...
		if (!empty($conf->mbisignature->enabled)) {
            ...
            $posy = $signatureArea->_signature_area_simple_or_double($pdf, $object, $posy, $outputlangs, $db, $object->ref, $langs, $this->page_largeur, $this->marge_droite, $this->emetteur->name);
		}
	}
}
